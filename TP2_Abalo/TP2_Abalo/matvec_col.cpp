// Produit matrice-vecteur, partition par colonnes
# include <cassert>
# include <vector>
# include <iostream>
#include <mpi.h>
...
int main(int argc , char *argv [] ) 
{
    ...
    for ( int i = 0; i < N; ++i ) {
        for (int j = 0; j < N_loc; ++j) {
            A_loc(i,j) = A(i,j+imin_loc);
        }
    }

    std::vector<double> u_loc( N_loc );
    for (int i = 0; i < N_loc; ++i) u_loc[i] = i+1+imin_loc;
    //Multiplication partielle pour chaque processus
    std::vector<double> v = A_loc*u_loc;
    std::cout << "Résultat partiel : " << v << "\n" << std::endl;
    // Somme  
    MPI_Allreduce (v.data(), recvbuf.data(), N, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD );
