...
std::vector<int> computeMandelbrotSet( int W, int H, int maxIter, int rank, int nbp ){
	...
	//Pour le process 0 qui est le root on lui laisse "l'image d'origine"
    //if(rank == 0){
		std::vector<int> pixels(W*H);
	//}
    start = std::chrono::system_clock::now();
	
	//Creer un vecteur pixels local
	int imin_loc = (H/nbp)*(nbp - rank - 1);
	int imax_loc = (H/nbp)*(nbp - rank);
	int H_loc = H/nbp;
    ...
    //sendcount=recvcount, pour un vector le passage par adresse ne se fait pas avec & mais plutot .data() 
    end = std::chrono::system_clock::now();
    std::chrono::duration<double> elapsed_seconds = end-start;
    std::cout << "Temps calcul ensemble mandelbrot : " << elapsed_seconds.count() 
              << std::endl;
    // Création d'image pour chaque processus
    // char name[128];
    // sprintf(name,"mandelbrot%03d.tga",rank);
    // savePicture(name, W, H_loc, pixels_loc, maxIter);
    return pixels;
}
...
int main(int argc, char *argv[] ) {
	MPI_Init( &argc, &argv );	
	//On definit les parametres du mpi
    int nbp, rank;
